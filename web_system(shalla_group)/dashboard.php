...
<div class="container">
    ...
    if ($result->num_rows > 0) {
        echo "<table><tr><th>Order ID</th><th>Item ID</th><th>Item Price</th><th>Quantity</th><th>Date Added</th><th>Item Name</th><th>Image URL</th></tr>";
        while($row = $result->fetch_assoc()) {
            echo "<tr><td>".$row["order_id"]."</td><td>".$row["item_id"]."</td><td>".$row["item_price"]."</td><td>".$row["quantity"]."</td><td>".$row["date_added"]."</td><td>".$row["item_name"]."</td><td>".$row["image_url"]."</td></tr>";
        }
        echo "</table>";
    } else {
        echo "0 results";
    ...
    if ($result->num_rows > 0) {
        echo "<table><tr><th>Item ID</th><th>Item Name</th><th>Price</th><th>Quantity On Hand</th><th>Item Category</th><th>Product Information</th><th>Image URL</th><th>Date Added</th></tr>";
        while($row = $result->fetch_assoc()) {
            echo "<tr><td>".$row["item_id"]."</td><td>".$row["item_name"]."</td><td>".$row["price"]."</td><td>".$row["quantity_onhand"]."</td><td>".$row["item_category"]."</td><td>".$row["product_information"]."</td><td>".$row["image_url"]."</td><td>".$row["date_added"]."</td></tr>";
        }
        echo "</table>";
    } else {
        echo "0 results";
